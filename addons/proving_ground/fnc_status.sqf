	...
	[] spawn {
		while {PG_get(STATUS)} do {
			...
			if ((alive _cursortarget)) then {
				...
				GET_CTRL(balca_hint_text2_IDC) ctrlSetText format ["Distance: %1",_cursortarget distance positionCameraToWorld [0,0,0]];
				if (((count _crew) > 0)and!(_cursortarget isKindOf "CAManBase")) then {
					...
					GET_CTRL(balca_hint_text3_IDC) ctrlSetText format ["Crew status: %1",_crew_stat];
				};
