...
_moneyObject = (nearestobjects [getpos player, ["Land_Sack_F"],  3] select 0);
...
{	
	...
	if(player distance (nearestobjects [player, ["Land_Sack_F"],  3] select 0) > 3) exitWith 
	{
		mutexScriptInProgress = false;
        _moneyObject setVariable ["owner", "world", true];
	};
	...
	{
		sleep 1;
		_currMoneyTemp = (nearestObjects [getpos player, ["Land_Sack_F"],  3]);
		...
		if(count _currMoneyTemp == 0) then 
		{
			...
			_money = ((nearestobjects [getpos player, ["Land_Sack_F"],  3] select 0) getVariable "money");
			deleteVehicle (nearestobjects [getpos player, ["Land_Sack_F"],  3] select 0);
